...
S3 support is tested against a real S3-compatible service rather than a mocked one, using [Testcontainers](https://testcontainers.com/) to start a disposable [S3Mock](https://github.com/adobe/S3Mock) container for the duration of the test. This exercises the real AWS SDK client, the real HTTP connection, and the real builder configuration (endpoint override, path-style access, credentials) end-to-end, which a mock cannot verify.
