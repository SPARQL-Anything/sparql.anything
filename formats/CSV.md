...
| csv.delimiter | It sets the column delimiter, usually ,;\t etc. | Any single character | , |
| csv.quote-char | It sets the quoting character | Any single character | &quot; |
| csv.null-string | It tells the CSV triplifier to not produce triples where the specified string would be in the object position of the triple | Any String | Not set |
| csv.ignore-columns-with-no-header | It tells the csv triplifier to ignore from the cells of columns having no headers. Note that if the property is set as true when csv.headers is false, the triplifier does not generate any slot (as no headers are collected). -- see #180 | true/false | false |
