...
public class AppTest {
	...
	public void testConstructAndSelect() throws IOException {
		...
		try {
			...
			Query q = QueryFactory.create("CONSTRUCT {?s ?p ?o} WHERE { SERVICE<facade-x:http://example.org/file.test>{?s ?p ?o}}");
			...
			Query select = QueryFactory.create("SELECT DISTINCT ?g ?s ?p ?o WHERE { SERVICE<facade-x:http://example.org/file.test>{GRAPH ?g {?s ?p ?o}}}");
